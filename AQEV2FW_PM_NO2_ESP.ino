...
#define EEPROM_MQTT_TOPIC_SUFFIX_ENABLED  (EEPROM_NO2_BASELINE_VOLTAGE_TABLE - 1)    // a simple flag to indicate whether or not the topic suffix is enabled
...
#define EEPROM_BACKUP_PM10P0_CAL_OFFSET  (EEPROM_BACKUP_NO2_CAL_OFFSET + 4)
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |    NO2 / PM Sensor Suite     |"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_no2_pm_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean add_baseline_voltage_characterization(char * arg, uint32_t eeprom_table_base_address) {
    ...
    if(!empty_location_found) {
        ...
        if(eeprom_table_base_address == EEPROM_NO2_BASELINE_VOLTAGE_TABLE) {
            Serial.print(F("no2"));
        }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"no2\",\"particulate\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
